...
// /**
//  * POST /api/mail/send
//  * 🔹 Sends mail via SMTP if `emailAccountId` is provided
//  * 🔹 Falls back to SendGrid if not
//  */
// router.post("/send", async (req, res) => {
//   try {
//     const {
...
//     // ✉️ Normalize attachments (URL or base64)
//     const preparedAttachments = await Promise.all(
//       attachments.map(async (att) => {
//         // If already base64 (from in-memory uploads)
//         if (att.content) {
//           return {
//             filename: att.name || att.filename || "file",
//             content: att.content,
...
//         // If file has URL, fetch and convert to base64
//         if (att.url) {
//           try {
//             const resp = await fetch(att.url);
//             const buf = await resp.arrayBuffer();
...
//     // 🔹 Prefer SMTP if account ID is provided
//     if (emailAccountId) {
//       const account = await prisma.emailAccount.findUnique({
//         where: { id: Number(emailAccountId) },
//       });
